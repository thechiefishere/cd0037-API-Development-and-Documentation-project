...
def get_token(user):
    ...
    token = jwt.encode(
        {'username': user['username'], 'exp': timeLimit}, os.getenv('SECRET_KEY'))
...
def create_app(test_config=None):
    ...
    def add_new_category():
        ...
        try:
            category = Category(type=body['type'])
            if not category:
                abort(422)
            category.insert()
    ...
    def search_questions():
        ...
        if 'searchTerm' not in body:
            print("am enter ere for search")
            abort(400)

        try:
            questions_with_search_term = Question.query.filter(
                Question.question.ilike('%' + body['searchTerm'] + '%')).all()
            paginated_questions = paginate_questions(
                questions_with_search_term, request)
    ...
    def get_questions_by_categories(id):
        try:
            ...
            category_id = paginated_questions[0]['category']
            category = Category.query.filter_by(
                id=str(category_id)).first().format()
    ...
    def get_quiz():
        ...
        try:
            ...
            formatted_questions_ids = [
                question.format()['id'] for question in category_questions]
    ...
    def login():
        ...
        try:
            user = User.query.filter_by(
                username=body['username']).first().format()
            if user is None:
                abort(401)
            is_password_correct = is_correct_password(
